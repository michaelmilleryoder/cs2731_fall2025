...
## Troubleshooting
* If you are not able to load `jupyterhub.crc.pitt.edu`, try using the Pitt VPN through the GlobalProtect app. Instructions for setting that up are [here](https://services.pitt.edu/TDClient/33/Portal/KB/ArticleDet?ID=293). This is necessary if you are off-campus or not on the WIRELESS-PITTNET network.
* If you can log in to `jupyterhub.crc.pitt.edu` but can't spawn a server, make sure you have enough space in your CRCD home directory. This could especially be an issue if you have used the CRCD before. Check your usage by using `ssh` to log in to `h2p.crc.pitt.edu` and running `crc-quota`.
